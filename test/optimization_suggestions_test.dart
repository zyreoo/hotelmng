...
void main() {
  ...
  group('generateOptimizationSuggestions', () {
    ...
    test('fillable gap generates CONTINUITY suggestion with concrete move (exact match)', () {
      // Room 101: gap 10–12 (2 nights). Room 102: booking 10–12 (same dates).
      final bookings = [
        _booking(id: 'b1', roomId: '101', checkIn: DateTime.utc(2025, 1, 5), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'b2', roomId: '101', checkIn: DateTime.utc(2025, 1, 12), checkOut: DateTime.utc(2025, 1, 16)),
        _booking(id: 'b3', roomId: '102', checkIn: DateTime.utc(2025, 1, 10), checkOut: DateTime.utc(2025, 1, 12)),
      ];
      final result = generateOptimizationSuggestions(
        bookings,
        windowStart: fakeNow,
      ...
      final continuity = result.where((s) => s.type == SuggestionType.continuity).toList();
    ...
    test('fillable gap does not also emit SHORT_GAP for same room (deduplication)', () {
      final bookings = [
        _booking(id: 'b1', roomId: '101', checkIn: DateTime.utc(2025, 1, 5), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'b2', roomId: '101', checkIn: DateTime.utc(2025, 1, 12), checkOut: DateTime.utc(2025, 1, 16)),
        _booking(id: 'b3', roomId: '102', checkIn: DateTime.utc(2025, 1, 10), checkOut: DateTime.utc(2025, 1, 12)),
      ...
      final continuity = result.where((s) => s.type == SuggestionType.continuity && s.roomId == '101').toList();
      final shortGap101 = result.where((s) => s.type == SuggestionType.shortGap && s.roomId == '101').toList();
      expect(continuity.length, 1);
      expect(shortGap101.length, 0);
    });

    test('suggests move only when it does not create new short gap in source (safety)', () {
      ...
      final bookings = [
        _booking(id: 'a1', roomId: '101', checkIn: DateTime.utc(2025, 1, 5), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'a2', roomId: '101', checkIn: DateTime.utc(2025, 1, 12), checkOut: DateTime.utc(2025, 1, 15)),
        _booking(id: 'b1', roomId: '102', checkIn: DateTime.utc(2025, 1, 8), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'b2', roomId: '102', checkIn: DateTime.utc(2025, 1, 10), checkOut: DateTime.utc(2025, 1, 12)),
        _booking(id: 'b3', roomId: '102', checkIn: DateTime.utc(2025, 1, 12), checkOut: DateTime.utc(2025, 1, 14)),
      ];
      final result = generateOptimizationSuggestions(
        bookings,
        windowStart: fakeNow,
      ...
      final continuityFor101 = result.where((s) => s.type == SuggestionType.continuity && s.toRoomId == '101').toList();
    ...
    test('partial fit inside gap produces CONTINUITY suggestion', () {
      ...
      final bookings = [
        _booking(id: 'a1', roomId: '101', checkIn: DateTime.utc(2025, 1, 20), checkOut: DateTime.utc(2025, 1, 23)),
        _booking(id: 'a2', roomId: '101', checkIn: DateTime.utc(2025, 1, 26), checkOut: DateTime.utc(2025, 1, 29)),
        _booking(id: 'b1', roomId: '102', checkIn: DateTime.utc(2025, 1, 24), checkOut: DateTime.utc(2025, 1, 26)),
      ];
      final result = generateOptimizationSuggestions(
        bookings,
        windowStart: fakeNow,
    ...
    test('exact-fill candidate scores better than partial-fit and is chosen', () {
      // Room 101: gap 23–26 (3 nights).
      // Room 102: exact 23–26 (3 nights). Room 103: partial 24–26 (2 nights).
      final bookings = [
        _booking(id: 'a1', roomId: '101', checkIn: DateTime.utc(2025, 1, 20), checkOut: DateTime.utc(2025, 1, 23)),
        _booking(id: 'a2', roomId: '101', checkIn: DateTime.utc(2025, 1, 26), checkOut: DateTime.utc(2025, 1, 29)),
        _booking(id: 'bExact', roomId: '102', checkIn: DateTime.utc(2025, 1, 23), checkOut: DateTime.utc(2025, 1, 26)),
        _booking(id: 'bPartial', roomId: '103', checkIn: DateTime.utc(2025, 1, 24), checkOut: DateTime.utc(2025, 1, 26)),
      ];
      final result = generateOptimizationSuggestions(
        bookings,
        windowStart: fakeNow,
    ...
    test('does not suggest moving the same booking twice across gaps (dedup by bookingId)', () {
      // Booking b1 could fit gaps in both 101 and 102, but should only be suggested once.
      final bookings = [
        // Room 101: gap 10–13
        _booking(id: 'r1a', roomId: '101', checkIn: DateTime.utc(2025, 1, 5), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'r1b', roomId: '101', checkIn: DateTime.utc(2025, 1, 13), checkOut: DateTime.utc(2025, 1, 16)),
        // Room 102: gap 10–13 as well
        _booking(id: 'r2a', roomId: '102', checkIn: DateTime.utc(2025, 1, 5), checkOut: DateTime.utc(2025, 1, 10)),
        _booking(id: 'r2b', roomId: '102', checkIn: DateTime.utc(2025, 1, 13), checkOut: DateTime.utc(2025, 1, 16)),
        // Candidate booking spanning 10–13 in room 103
        _booking(id: 'b1', roomId: '103', checkIn: DateTime.utc(2025, 1, 10), checkOut: DateTime.utc(2025, 1, 13)),
      ];
      final result = generateOptimizationSuggestions(
        bookings,
        windowStart: fakeNow,
